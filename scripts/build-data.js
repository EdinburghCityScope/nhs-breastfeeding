...
const csvFilename = 'breastfeeding.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?dataCollectionTime ?group ?measure ?value
            WHERE {
                ?s qb:dataSet data:breastfeeding ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:breastfeedingDataCollectionTime ?t ;
                   dim:populationGroup ?g ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?g rdfs:label ?group .
                ?z rdfs:label ?zone .
                ?t rdfs:label ?dataCollectionTime .
                ?y rdfs:label ?year .
